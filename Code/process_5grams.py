...
FINAL_PATTERN_5_WORDS = re.compile(
    r"""
    ^(?:
...
def is_gendered_format(line):
    return bool(FINAL_PATTERN_5_WORDS.match(line))
...
def parse_fivegrams(year_start=1900, year_end=2019):
    t0 = time.time()
    years = np.arange(year_start, year_end + 1)
    countDict = defaultdict(lambda: np.zeros_like(years))
    print('STARTING...')
    for i in range(19423):
        num = f'{i:05}'
        url = f'http://storage.googleapis.com/books/ngrams/books/20200217/eng/5-{num}-of-19423.gz'
        read_file(url, countDict, years)
        if i % 100 == 0:
            print(f'Processed {i+1} files')
            print(f'Time elapsed: {(time.time() - t0)/60:.2f} minutes')
            print('SAVING PROGRESS...')
            with open('csvs/fivegrams-gendered.csv', 'w', encoding='utf-8', newline="") as f:
    ...
    print('DONE!')
    print(f'Time elapsed: {(time.time() - t0)/60:.2f} minutes')
    with open('csvs/fivegrams-gendered-final.csv', 'w', encoding='utf-8', newline="") as f:
        writer = csv.writer(f, delimiter=",")
        writer.writerow(['PHRASE', *years])
        for word, counts in countDict.items():
            writer.writerow([word, *counts])
